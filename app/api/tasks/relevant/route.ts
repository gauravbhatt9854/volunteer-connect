...
export async function GET() {
  try {
    ...
    for (const task of tasks) {
      ...
      /* ---------- FINAL SCORE ---------- */
      /* ---------- FINAL SCORE ---------- */
      let finalScore = aiScore + locWeight;

      // ✅ cap at 1 (so never > 100%)
      finalScore = Math.min(finalScore, 1);

      /* convert to % with 2 decimals */
      const finalScorePercent = Number((finalScore * 100).toFixed(2));
      ...
      results.push({
        ...
        similarityScore: finalScorePercent, // ✅ percentage
      });
